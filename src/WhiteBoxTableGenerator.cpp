...
WhiteBoxTableGenerator::WhiteBoxTableGenerator(
  ...
  if (use_mixing_bijections && !use_internal_encoding) {
    ...
    for (size_t x = 0; x <= std::numeric_limits<uint8_t>::max(); ++x) {
      ...
      if (i % 4 == 3) {
        for (size_t k = 0; k < 8; ++k) {
          auto zero_at_pos = static_cast<uint32_t>((x & (1U << k)) != 0);
          v.put(k, zero_at_pos);
        }
      } else if (i % 4 == 2) {
        for (size_t k = 8; k < 16; ++k) {
          auto zero_at_pos = static_cast<uint32_t>((x & (1U << (k - 8))) != 0);
          v.put(k, zero_at_pos);
        }
      } else if (i % 4 == 1) {
        for (size_t k = 16; k < 24; ++k) {
          auto zero_at_pos = static_cast<uint32_t>((x & (1U << (k - 16))) != 0);
          v.put(k, zero_at_pos);
        }
      } else if (i % 4 == 0) {
        for (size_t k = 24; k < 32; ++k) {
          auto zero_at_pos = static_cast<uint32_t>((x & (1U << (k - 24))) != 0);
          v.put(k, zero_at_pos);
        }
      }
  ...
  for (uint32_t i = 0; i < AES_KEY_LENGTH_BYTES; ++i) {
    ...
    for (uint32_t j = 0; j <= std::numeric_limits<uint8_t>::max(); ++j) {
      ...
      if (add_input_encoding) {
        auto x = static_cast<uint8_t>((j & 0xf0U));
        x >>= 4U;
        auto y = static_cast<uint8_t>(j & 0xfU);
      ...
      if (add_output_encoding) {
        ...
        auto nibble_7 = static_cast<uint8_t>((result & 0xf0U) >> 4);
        auto nibble_6 = static_cast<uint8_t>((result & 0xf00U) >> 8);
        auto nibble_5 = static_cast<uint8_t>((result & 0xf000U) >> 12);
        auto nibble_4 = static_cast<uint8_t>((result & 0xf0000U) >> 16);
        auto nibble_3 = static_cast<uint8_t>((result & 0xf00000U) >> 20);
        auto nibble_2 = static_cast<uint8_t>((result & 0xf000000U) >> 24);
        auto nibble_1 = static_cast<uint8_t>((result & 0xf0000000U) >> 28);
        ...
        result = (nibble_1 << 28U) | (nibble_2 << 24U) | (nibble_3 << 20U) |
                 (nibble_4 << 16U) | (nibble_5 << 12U) | (nibble_6 << 8U) |
                 (nibble_7 << 4U) | nibble_8;
      }
...
void WhiteBoxTableGenerator::encodeTyiTablesDecryption(
  ...
  for (uint32_t i = 0; i < AES_KEY_LENGTH_BYTES; ++i) {
    ...
    for (uint32_t j = 0; j <= std::numeric_limits<uint8_t>::max(); ++j) {
      ...
      if (add_output_encoding) {
        // Get all 8 nibbles of result
        auto nibble_8 = static_cast<uint8_t>(result & 0xfU);
        auto nibble_7 = static_cast<uint8_t>((result & 0xf0U) >> 4);
        auto nibble_6 = static_cast<uint8_t>((result & 0xf00U) >> 8);
        auto nibble_5 = static_cast<uint8_t>((result & 0xf000U) >> 12);
        auto nibble_4 = static_cast<uint8_t>((result & 0xf0000U) >> 16);
  ...
  if (xor_tables != nullptr) {
    for (size_t i = 0; i < 9; ++i) {
      for (size_t j = 0; j < 96; ++j) {
        for (size_t x = 0; x <= std::numeric_limits<uint8_t>::max(); ++x) {
          auto lower_nibble = static_cast<uint8_t>(x & 0xFU);
          auto upper_nibble = static_cast<uint8_t>(x & 0xF0U);
          (*xor_tables)[i][j][x] = (upper_nibble >> 4U) ^ lower_nibble;
        }
      }
    }
  }
